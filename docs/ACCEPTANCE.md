...
| ID | 层级 | 输入 / 故障 | 期望 | 执行位置 |
|---|---|---|---|---|
| D01 | 源图人工核对 | p19概述与p57详细图 | 记录12/13差异并采用13段，不杜撰一致 | SOURCE_MAP / DECISIONS |
| D04 | 设计走查 | 订单A明确改成B | 同question新实体版本；旧审批失效 | 设计基线；运行版M04/M10/M15 |
...
视角 A 仅用 CONTRACTS 编写“缺订单”请求/响应草案；视角 B 仅用 ARCHITECTURE 及 M00 场景写相同流程的状态/工具次数预期。对比 Question=WAITING_SLOT、outcome=CLARIFY、run 正常结束、tool_calls=0、下一消息不是 resume。再对跨用户拒绝和重复审批做相同对照。必要时可用两个独立会话，但必须记录实际输入/输出，不能把单会话自查称为独立审查。

M00 已通过文档走查确定契约，不要求为此启动业务中间件；但不能把本矩阵未来行改成 PASS 来凑模块退出条件。后续代码改变任一语义时，要同时回归另一视角和对应运行用例。
