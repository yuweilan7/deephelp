...
版本：`M00-design-v1.2`；语义契约：`0.1.2-draft`。这是 M00 工程设计基线，不是运行中系统描述。验收场景见 `ACCEPTANCE.md`，来源裁决见 `DECISIONS.md`，当前实现状态见 `PROJECT_STATE.md`。
...
tests/{unit,integration,live,e2e}/    # M01起；测试随实现交付
configs/                             # 可公开模板，不含凭据
scripts/                             # 小型运维/导入/评测命令，不建新平台
planning/Mxx/                       # 当前模块生成的PLAN及必要S；未规划时可不存在
...
审批用例用CAS保证同一PENDING决定只迁移一次，执行边界在短事务中校验计划快照并唯一领取PREPARED操作。绑定值在领取前变化使旧批准失效；本次领取自身推进Question/Run状态产生的新version记录到执行证据，不使本次领取自我失效。已领取后更正不得取消已发出的效果，需冻结原计划并对账。重复审批先鉴权，再返回同operation/run的既有结果或进行中引用；不得重复领取、写工具或新增业务run。普通“好的”可只澄清，但原审批和原run保持等待。详见ADR018和CONTRACTS的审批补充。
...
M00 通过标准：三个指定走查能用同一套状态/ID/错误/版本解释；两份接口视角草案兼容；未定项有后续门禁；不存在按旧启动包重装或加库的指令。细项见 `ACCEPTANCE.md`。

## v2边界补充

上下文中的开放question包括ACTIVE、WAITING_SLOT，WAITING_APPROVAL仅供受控上下文读取。普通消息不恢复审批run；更正计划与执行领取需要版本/事务协调，已进入外部执行的操作不能被口头视为已撤销，必须按结果对账。

M08负责最小MySQL消息/执行/问题账本和幂等，M10增量扩展而非重建。M11数据库失败时普通有状态请求失败关闭，只有明确无持久化的只读预览可仅用当前消息。M16的关键事实从结构化字段/模板输出，不能仅靠另一个LLM证明自然语言没有改义。

开发协作遵守 `AGENTS.md` 与 `PLAN_MODULE_PROMPT.md`：按需拆分，一个 M 由一个主写者顺序实施，独立审查针对固定 commit。任务组织与运行时并发是两回事，不因此取消应用内部必要的异步 I/O。
